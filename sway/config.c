...
static void config_defaults(struct sway_config *config) {
	if (!(config->swaynag_command = strdup("swayngnag"))) goto cleanup;
	...
	config->swaynag_config_errors.args = "--type error "
			...
			"--detailed-message "
			"--button-no-terminal 'Exit swayng' 'swayngmsg exit' "
			"--button-no-terminal 'Reload swayng' 'swayngmsg reload'";
...
static char *get_config_path(void) {
	...
	struct config_path config_paths[] = {
		...
		{ .prefix = config_home, .config_folder = "swayng"},
		{ .prefix = home, .config_folder = ".i3"},
		{ .prefix = config_home, .config_folder = "i3"},
		{ .prefix = SYSCONFDIR, .config_folder = "swayng"},
